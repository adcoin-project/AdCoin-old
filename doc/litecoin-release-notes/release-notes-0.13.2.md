AdCoin version 0.13.2 is now available from:
  ...
  <https://download.adcoin.org/adcoin-0.13.2.1/>
...
Please report bugs using the issue tracker at github:
  ...
  <https://github.com/adcoin-project/adcoin/issues>
...
updates will be released anymore. Without security updates, using a adcoin
...
ECDSA signatures inside AdCoin transactions now use validation using
...
This allows running adcoind without having to do any manual configuration.
...
AdCoind can now (optionally) asynchronously notify clients through a
...
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals AdCoin
...
`adcoin.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
caching. A sample config for apache2 could look like:
    ...
    <Location /adcoinrpc>
        ...
        # optional bypass adcoind rpc basic auth
        # RequestHeader set Authorization "Basic <hash>"
        # get the <hash> from the shell with: base64 <<< adcoinrpc:<password>
    </Location>
...
Restarting adcoind will no longer clear out the list of banned peers; instead
...
- Adding `dbcache=100` in adcoin.conf
...
For example:
    ...
    $ src/adcoin-cli -stdin walletpassphrase
    mysecretcode
    120
...
- `adcoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries for the most
  common 32-bit ARM architecture.
- `adcoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries for the most
...
for a new opcode in the AdCoin scripting system that in combination with
...
AdCoin transactions currently may specify a locktime indicating when
...
The two phases are distinct in the adcoin-qt GUI. During the first one,
...
- **Elimination of unwanted transaction malleability:** Segregating the witness
  ...
  malleability, which is a problem that makes programming AdCoin wallet
  software more difficult and which seriously complicates the design of smart
  contracts for AdCoin.
...
- **Weighting data based on how it affects node performance:** Some parts of
  each AdCoin block need to be stored by nodes in order to validate future
...
- **Signature covers value:** A simple improvement in the way signatures are
  ...
  of adcoins they think they are spending, and by having full nodes refuse to
  accept those signatures unless the amount of adcoins being spent is exactly
...
- **Increased security for multisig:** AdCoin addresses (both P2PKH addresses
  ...
  believed to be provided by AdCoin's choice of parameters for its Elliptic
...
- **More efficient almost-full-node security** Satoshi Nakamoto's original
  ...
  produce an accurate copy of AdCoin's current ledger (called the UTXO set),
...
- **Script versioning:** Segwit makes it easy for future soft forks to allow
  AdCoin users to individually opt-in to almost any change in the AdCoin
...
- `adcoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries targeting
  the 32-bit ARMv7-A architecture.
- `adcoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries targeting
