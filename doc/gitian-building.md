...
- [Building AdCoin](#building-adcoin-core)
...
Clone the git repositories for adcoin and Gitian.
...
in the adcoin repository under 'Fetch and create inputs' to install sources which require
...
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the adcoin repository.
...
Output from `gbuild` will look something like
    ...
    From https://github.com/adcoin-project/adcoin
...
For example:
```bash
URL=https://github.com/thrasher-/adcoin.git
...
and the adcoin git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../adcoin/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/adcoin
SIGPATH=/some/root/path/adcoin-detached-sigs

./bin/gbuild --url adcoin=${BTCPATH},signature=${SIGPATH} ../adcoin/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/adcoin-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/adcoin-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/adcoin-osx-build.assert
```
...
[adcoin-project/gitian.sigs.ltc](https://github.com/adcoin-project/gitian.sigs.ltc/) repository, or if that's not possible create a pull
